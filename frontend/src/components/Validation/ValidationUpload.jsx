...
export default function ValidationUpload({ onDone }) {
  ...
  return (
    <div className="val-upload">
      ...
      >
        <input ref={inputRef} type="file" accept=".pdf" style={{ display: 'none' }} onChange={e => e.target.files[0] && upload(e.target.files[0])} />
        {uploading ? <p>Processing with AI…</p> : <><p className="drop-icon">🏛️</p><p>Drop PDF here or click to browse</p></>}
      </div>
